...
--  SERVICES & CONSTANTS
-- ═══════════════════════════════════════
print("[idk hub] >>>> ck 1: getting services...")-- idk hub (v6) — Obsidian UI Library native
-- Preserves all original logic: anti-kick, luck machine renewer, animated sprite-sheet icon.
...
local function applyNowPlaying(playerState, queuePreview)
    ...
    -- Update Obsidian tab labels. Text labels first (these never throw), so the
    -- track/artist info shows even if the image set below it errors. Previously
    -- `spotifyArtImage:SetImage(artUrl)` came BEFORE the label sets, and when
    -- Obsidian's SetImage rejected a URL it threw inside `applyNowPlaying` —
    -- which the outer pcall caught but then aborted, leaving track/artist blank
    -- while the already-set sync globals kept the progress bar interpolating.
    ...
    -- Album art on the Obsidian tab image. Isolate in its own pcall so a rejected
    -- URL (Obsidian validates `Image` strictly) cannot abort the rest of the
    -- snapshot — the track/artist text has already been written above.
    if spotifyArtImage and artUrl ~= "" then
        pcall(function() spotifyArtImage:SetImage(artUrl) end)
    end

    -- Update popup labels
    popupTrackLabel.Text  = trackName
    popupArtistLabel.Text = artistStr
    if artUrl ~= "" then pcall(function() popupArt.Image = artUrl end) end
